...
class StringTranslationsResource(BaseResource):
	...
	def list_language_translations(
		...
		params = {
			"orderBy": orderBy,
			"stringIds": None if stringIds is None else ",".join(str(stringId) for stringId in stringIds),
			"labelIds": None if labelIds is None else ",".join(str(labelId) for labelId in labelIds),
			"fileId": fileId,
			"branchId": branchId,
