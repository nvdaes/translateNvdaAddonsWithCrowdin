...
class EnterpriseReportsResource(BaseReportsResource, BaseReportSettingsTemplatesResource):
	...
	def _prepare_stepTypes(step_types_const: dict, stepTypes: Optional[Iterable[StepTypes]] = None):
		if isinstance(stepTypes, list):
			stepTypes = [{**item, **step_types_const} for item in stepTypes if isinstance(item, dict)]
	...
	def download_report_archive(self, archiveId: int, exportId: str):
		...
		"""
		path = str(
			self.get_report_archive_export_path(archiveId=archiveId, exportId=exportId) + "/download",
		)
		return self.requester.request(
			method="get",
			path=path,
